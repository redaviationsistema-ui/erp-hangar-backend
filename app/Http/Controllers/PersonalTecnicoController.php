...
use App\Http\Resources\PersonalTecnicoResource;
use App\Models\Area;
...
class PersonalTecnicoController extends Controller
{
    public function index(Request $request)
    {
        ...
        $payload = $this->cacheOrFetch(
            ...
            function () use ($request) {
                ...
                return [
                    'success' => true,
                    'data' => PersonalTecnicoResource::collection($personal->getCollection())->resolve(),
                    'meta' => $this->meta($personal),
                ];
            }
        );
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'email' => 'nullable|email|max:255|unique:personal_tecnico,email',
        ...
        $personal = PersonalTecnico::create([
            ...
            'tipo' => $this->normalizeTipo($data['tipo'] ?? 'tecnico'),
            'estado' => $data['estado'] ?? 'Activo',
            'notas' => $data['notas'] ?? null,
        ])->load(['area:id,codigo,nombre', 'usuario:id,name,email']);

        $this->bustCache();

        return response()->json([
            'success' => true,
            'message' => 'Personal tecnico creado correctamente.',
            'data' => PersonalTecnicoResource::make($personal)->resolve(),
        ], 201);
    }

    public function show(Request $request, PersonalTecnico $personalTecnico)
    {
        $this->authorizeManagement($request);

        $payload = $this->cacheOrFetch(
            $this->cacheKey('show', ['id' => $personalTecnico->getKey()]),
            now()->addMinutes(5),
            function () use ($personalTecnico) {
                $personalTecnico->load(['area:id,codigo,nombre', 'usuario:id,name,email']);

                return [
                    'success' => true,
                    'data' => PersonalTecnicoResource::make($personalTecnico)->resolve(),
                ];
            }
        );
    ...
    public function update(Request $request, PersonalTecnico $personalTecnico)
    {
        $this->authorizeManagement($request);
        ...
        $data = $request->validate([
            ...
            'email' => 'sometimes|nullable|email|max:255|unique:personal_tecnico,email,'.$personalTecnico->getKey(),
        ...
        foreach (['nombre', 'email', 'telefono', 'puesto', 'especialidad', 'estado', 'notas'] as $field) {
        ...
        $personalTecnico->update($payload);
        $personalTecnico->load(['area:id,codigo,nombre', 'usuario:id,name,email']);
        $this->bustCache();

        return response()->json([
            'success' => true,
            'message' => 'Personal tecnico actualizado correctamente.',
            'data' => PersonalTecnicoResource::make($personalTecnico)->resolve(),
        ]);
    }

    public function destroy(Request $request, PersonalTecnico $personalTecnico)
    {
        $this->authorizeManagement($request);

        $personalTecnico->delete();
        $this->bustCache();

        return response()->json([
            'success' => true,
            'message' => 'Personal tecnico eliminado correctamente.',
        ]);
    }
    ...
    private function authorizeManagement(Request $request): void
    {
        ...
        $allowed = in_array($user?->rol, ['admin', 'supervisor'], true)
            ...
            || in_array('personal_tecnico.manage', $permissions, true)
            || in_array('manage_technical_staff', $permissions, true)
            || in_array('usuarios_crud', $permissions, true)
            || in_array('manage_users', $permissions, true);

        abort_unless($allowed, 403, 'No tienes permisos para gestionar personal tecnico.');
    }
    ...
    private function normalizeTipo(string $tipo): string
    {
        ...
        return $normalized === '' ? 'tecnico' : $normalized;
    }
    ...
    private function cacheKey(string $action, array $params = []): string
    {
        ...
        return 'personal_tecnico:'.Cache::get('personal_tecnico_cache_version', 1).':'.$action.':'.md5(json_encode($params));
    }

    private function bustCache(): void
    {
        Cache::forever('personal_tecnico_cache_version', (int) Cache::get('personal_tecnico_cache_version', 1) + 1);
    }
}
